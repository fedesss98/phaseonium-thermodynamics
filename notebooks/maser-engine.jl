...
#> [frontmatter]
#> title = "Maser Thermal Engine"

using Markdown
using InteractiveUtils

# ╔═╡ 450a5474-58a0-4c76-b708-0cc48789c403
using ProgressBars

# ╔═╡ bc43aa84-b3e7-4eb3-a979-a3508afa1d5e
using Plots

# ╔═╡ 2048e34b-8d04-4580-a513-03c697ae8d4d
using Serialization

# ╔═╡ 575c6be0-6b5d-11ef-1182-4798726821b8
import QuantumOptics

# ╔═╡ 89b7236d-03f7-421c-a053-e175a09d8508
import LinearAlgebra

# ╔═╡ 50470b7b-28d9-4a88-b79b-e806d70a0882
const NDIMS = 100

# ╔═╡ 2c5253a9-448c-4ad1-8f30-4fa8b9267160
const L0 = 10.0

# ╔═╡ c9dba18b-a0c0-4d8b-9a1e-072a35d0fde0
const V = 1e-2

# ╔═╡ 321ad4d3-0cd7-42e7-a7a8-cefff6ce7686
const α0 = 2*pi

# ╔═╡ 30efbb6f-ef47-4494-baa1-ddd17cdea610
const T_c = 1.0

# ╔═╡ aed751dc-c78f-4146-a8f4-0dbfeb9eb45f
const T_h = 10.0

# ╔═╡ 6e51ca53-81f1-426e-80e7-26a2994edb1e
const STROKE_TIME = 500

# ╔═╡ 52b062c6-edfc-4d7f-a02a-89bd2fcee2ae
const MAX_CYCLES = 2

# ╔═╡ ab633775-288a-4de8-9559-8c47aba9d497
boltzmann_population(omega, temp) = 1 / (ℯ^(omega / temp) + 1)

# ╔═╡ 518400ef-5ea8-4c20-841c-360eb8d966ac
# Maximum and minimum lengths reached by the cavity
max_len = L0 + V * STROKE_TIME

# ╔═╡ c7108d5d-3eaf-438a-abcd-2ce803dda6d9
min_len = L0 - V * STROKE_TIME

# ╔═╡ ebc6dcc1-1181-4066-a109-4e85d0118f12
...
# ╔═╡ 0d63d185-60cb-4656-9d1d-b918800aa6b6
begin
	t_values = 1:10*STROKE_TIME
	y_values = [cavity_len(t) for t in t_values]
	
	# Plot the function
	plot(t_values, y_values, label="Periodic Function", xlabel="Time", ylabel="Value", title="Periodic Function Plot")
end

# ╔═╡ 8976cb65-2848-4e7f-9f5e-0bf2ac640697
# Frequencies of the cavity
ω = α0 / L0

# ╔═╡ b591d50c-9a8d-4844-84c8-24e8fe591868
ω_cold = α0 / max_len

# ╔═╡ 4322f56f-a8a8-4220-a3c0-31a19f424f98
ω_hot = α0 / min_len

# ╔═╡ 1d848c2b-7b9f-41d4-a83a-a2bddb80c4b1
# Populations of the cavity in the cold and hot baths
p_cold = boltzmann_population(ω_cold, T_c)

# ╔═╡ 4cf3376c-76d8-4067-acec-d62cf8f69345
p_hot = boltzmann_population(ω_hot, T_h)

# ╔═╡ 5e9f73db-1f02-4990-980d-96e83f724223
# Operators and Hamiltonian
basis = QuantumOptics.FockBasis(NDIMS)

# ╔═╡ 85001dcb-d52e-4682-8b81-80313a261151
a = QuantumOptics.destroy(basis)

# ╔═╡ 80be3190-9b41-4a7f-a5cd-d538046ce659
ad = QuantumOptics.create(basis)

# ╔═╡ 9ff7d767-02fb-4554-9835-44c8379eed6c
n = QuantumOptics.number(basis)

# ╔═╡ 0dcd6aed-b784-40fd-8c48-11dfa7503cb5
Hbase = α0 * ( n + QuantumOptics.identityoperator(basis)*1/2 )

# ╔═╡ 3b619a29-da70-43db-a9dd-9aa92e8dd7d0
# Time-dependent Hamiltonian
function H(t)
	Hbase / cavity_len(t)
end

# ╔═╡ a814ea89-cac9-4f05-8463-df42d459f1b3
try
...
# ╔═╡ 01b3ba0c-447a-49f8-a3aa-dca5eb490456
systems = []

# ╔═╡ e216bffc-c1d9-40f1-9174-27254f9becfe
push!(systems, system)

# ╔═╡ eea4e80c-d26d-403c-b6ed-2ea1bbfbe770
function truncation_valid(system)
	return real(system.data[end, end]) < 1e-4
end

# ╔═╡ f198c24c-67f9-47d5-bb4c-8b1b6895e96f
truncation_valid(system)

# ╔═╡ 97694ff4-e34e-4f4d-98de-bba630e0007e
"""
...
"""
function temperature_of_system(state, hamiltonian)
...
# ╔═╡ a224dac0-6a58-4025-a6eb-d18cee3866f4
temperature_of_system(system, H(0))

# ╔═╡ 2ea1304b-cad7-4c1b-8ff9-7052e70fc556
"""
...
"""
function adiabatic_compression(system0; time::Int64=0, stroke_time=STROKE_TIME)
	system = system0
	entropies = zeros(eltype(system), stroke_time)
	temperatures = zeros(eltype(system), stroke_time)
	for t in 1:stroke_time
		h = H(t + time)
		try
			entropies[t] = QuantumOptics.entropy_vn(system, tol=1e-18)
			temperatures[t] = temperature_of_system(system, h)
		catch e
...
# ╔═╡ 02a9df09-5d1c-4200-9a9e-cbf74c432fe9
function plot_first_adiabatic(system, time)
	...
	plot(entropies, temperatures)
end

# ╔═╡ 3aa0528e-8bea-4ce3-9cc4-be38708d96f5
# Uncomment following line to check the Adiabatic Stroke is working properly
#plot_first_adiabatic(system, 1)

# ╔═╡ 9fccf295-24d7-4a37-99b0-bc2c7d193b39
function heating(system, rate, ad, a; time::Int64=0, stroke_time=STROKE_TIME)
	t = [1:stroke_time;]
	h = H(time)
...
# ╔═╡ 8259ebf3-2e99-4ad2-904d-b9880dbd5e91
...
# ╔═╡ e0d6625e-e93c-40b6-b1c1-2dc35159d1b7
function plot_e_vs_t(entropies, temperatures)
	# Plot entropy vs. temperature
	entropies = round.(real.(entropies), digits=4)
	temperatures = round.(real.(temperatures), digits=4)
	plot(entropies, temperatures, ylabel="Temperature (K)", xlabel="Entropy", title="Entropy vs. Temperature")
end

# ╔═╡ 588b441f-cd51-49a8-a13e-0f0267b908f8
# FULL CYCLE
function cycle(tspan, system0)
	...
	for t in 1:tspan		
		...
		# Isochoric Cooling
		system = heating(system, p_cold, ad, a)
		current_time += STROKE_TIME
		add_system_measure(system, current_time, temperatures, entropies)
	end
...
# ╔═╡ 32d65b59-2cd0-4b92-a8de-562f4f35213e
if stabilized
...
# ╔═╡ 46038909-f3cc-4fa5-957e-0b4ab5dbab1f
temp_cycles

# ╔═╡ 8ebd890a-bf07-4221-b67b-f0684a9edaeb
entrop_cycles

# ╔═╡ ed3936a1-aae3-4661-9539-0be82f059bd1
plot(entrop_cycles, temp_cycles)

# ╔═╡ c05308cf-4008-416b-9bd2-afd6ac7e784a
temperature_of_system(system, H(0))

# ╔═╡ 8d125d71-dcf5-46e2-ba39-073ef8923ecc
open("starting_state_maser.dat", "w") do f
	serialize(f, system_stable)
end

# ╔═╡ 366118fd-41b4-4fef-97ef-b254425e7843
open("starting_state_maser.dat", "r") do f
	s = deserialize(f)
end

# ╔═╡ 4a4ec3e2-54aa-4b5f-ac52-b5d728a162e7
function phaseonium_heating(system, γα, γβ, ad, a; time::Int64=0, stroke_time=STROKE_TIME)
	t = [1:stroke_time;]
	h = H(time)
	tout, systems = QuantumOptics.timeevolution.master(t, system, h, [ad, a], rates=[γα, γβ])
	return systems[end]
end

# ╔═╡ f17e0e08-03a2-46ee-942f-c544917915dd
# FULL PHASEONIUM CYCLE
function phaseonium_cycle(tspan, system0)
	system = system0
	temperatures = [round.(real.(temperature_of_system(system, H(0))), digits=6);]
	entropies = [round.(real.(QuantumOptics.entropy_vn(system)), digits=6);]
	current_time = 0
	for t in 1:tspan		
		...
		# Isochoric Heating
		system = phaseonium_heating(system, p_hot, ad, a)
		current_time += STROKE_TIME
		add_system_measure(system, current_time, temperatures, entropies)
		...
		# Isochoric Cooling
		system = phaseonium_heating(system, p_cold, ad, a)
		current_time += STROKE_TIME
		add_system_measure(system, current_time, temperatures, entropies)
	end
	println("Final time $current_time")
...
# ╔═╡ db7ffb6f-8113-4e78-8f19-7a92836a452f
"""
Implement the fully-coherent phaseonium Master Equation for one cavity.
"""
function phaseonium_stroke(system, parameters, a, ad; time::Int64=0, stroke_time=STROKE_TIME)
	t = [1:stroke_time;]
...
# ╔═╡ 0ee36b16-9fbc-4f50-bb2b-267d3c2189f7
function set_parameters(;α²=1/4, ϕ=π/2, χ₁=π/4, χ₂=π/4)
	β²= 1-α²
	return α², β², ϕ, χ₁, χ₂
end

# ╔═╡ 0d96c706-e9d5-42a1-8e80-70b394a5f952
function plot_phaseonium_stroke(parameters; t::Int64 = 400)
	temp_0 = round.(real.(temperature_of_system(system, H(0))), digits=10)
	entropy_0 = round.(real.(QuantumOptics.entropy_vn(system)), digits=10)
	systems_evolved = phaseonium_stroke(system, parameters, a, ad, stroke_time=t)
	s_evolution = [entropy_0;]
	t_evolution = [temp_0;]
	for s in systems_evolved
		temp = round.(real.(temperature_of_system(s, H(0))), digits=10)
		entropy = round.(real.(QuantumOptics.entropy_vn(s)), digits=10)
		push!(t_evolution, temp)
		push!(s_evolution, entropy)
	end
	if !(truncation_valid(systems_evolved[end]))
		error = real(systems_evolved[end].data[end, end])
		throw(ErrorException("Hilbert truncation no more valid: last element = $error"))
	end
	last_element = systems_evolved[end].data[end][end]
	println("Last element: $last_element")
	println(length(systems_evolved))
	plot(s_evolution, t_evolution)
	return s_evolution, t_evolution
end

# ╔═╡ 4ae21277-43bf-402c-aba6-9e2b8165a410
begin
	α² = 0.25
	ϕ = π/4
	χ₁ = 0
	parameters = set_parameters(α²=α², ϕ=ϕ, χ₁=χ₁)
	...
	time = 1000
	try
		s_evolution, t_evolution = plot_phaseonium_stroke(parameters; t=time)
	catch exception
		s_evolution = [0]
		t_evolution = [0]
		println("Invalid parameter configuration.\n$exception")
	end
	plot([1:time+1;], t_evolution)
end

# ╔═╡ 557b22c0-cdb2-4710-8194-442123f74a45
time_to_plot = [range(1, time+1);]


# ╔═╡ e97f864d-5516-4854-9825-f267d116ce62
plot(time_to_plot, t_evolution)

# ╔═╡ 2a2c26f8-cbfe-468d-bf9a-23ab27d195f7


# ╔═╡ 41ad45f2-73c0-4126-8005-0a446455276a


# ╔═╡ c651d98e-9f58-4f7a-a800-b3e427b21274
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
α²=0.1, ϕ=0.5235987755982988, χ₁=0
Last element: -1.944807336397769e-13 + 5.55768694850605e-26im
10
  ╠═╡ =#
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "5b3e1740dce3dd2c65a7eb2c9c6decf48d75d73c"
...
deps = ["LinearAlgebra", "SparseArrays"]
git-tree-sha1 = "87d036956ea801cffed598b2fc765453493a3d41"
...
git-tree-sha1 = "460f4576de23a01aeb013dd504502c972df5ab77"
...
git-tree-sha1 = "35fa008783ddbbd4939b3dfb815503b5b038d982"
